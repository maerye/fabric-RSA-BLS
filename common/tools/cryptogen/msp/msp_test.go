...
const (
	...
	testOrganizationalUnit = "COP"
	testStreetAddress      = "testStreetAddress"
	testPostalCode         = "123456"
)

var testDir = filepath.Join("./", "msp-test")
...
func TestGenerateLocalRSAMSP(t *testing.T) {

	cleanup(testDir)

	err := msp.GenerateLocalRSAMSP(testDir, testName, nil, &ca.CA{}, &ca.CA{}, msp.PEER, true)
	assert.Error(t, err, "Empty CA should have failed")
	...
	// generate local MSP for nodeType=PEER
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.PEER, true)
	assert.NoError(t, err, "Failed to generate local MSP")
	...
	// generate local MSP for nodeType=CLIENT
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.CLIENT, true)
	assert.NoError(t, err, "Failed to generate local MSP")
	//only need to check for the TLS certs
	tlsFiles = []string{
		filepath.Join(tlsDir, "ca.crt"),
	...
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
	signCA.Name = "test/fail"
	err = msp.GenerateLocalRSAMSP(testDir, testName, nil, signCA, tlsCA, msp.ORDERER, true)
	assert.Error(t, err, "Should have failed with CA name 'test/fail'")
//	t.Log(err)
	//cleanup(testDir)

}
func TestGenerateLocalBLSMSP(t *testing.T) {
	...
	assert.NoError(t, err, "Error generating CA")
	// generate TLS CA
	tlsCA, err := ca.NewBLSCA(tlsCADir, testCAOrg, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")
...
func TestGenerateRSAVerifyingMSP(t *testing.T) {
	...
	err = msp.GenerateRSAVerifyingMSP(mspDir, signCA, tlsCA, true)
	assert.NoError(t, err, "Failed to generate verifying MSP")
...
func TestGenerateBLSVerifyingMSP(t *testing.T) {
	...
	err = msp.GenerateBLSVerifyingMSP(mspDir, signCA, tlsCA, true)
	assert.NoError(t, err, "Failed to generate verifying MSP")
