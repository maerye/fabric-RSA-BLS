...
var testDir = filepath.Join("./", "ca-test")
...
func TestLoadCertificateBLS(t *testing.T) {
	caDir := filepath.Join(testDir, "ca")
	certDir := filepath.Join(testDir, "certs")
	// generate private key
	priv, _, err := csp.GenerateBLSPrivateKey(certDir)
	assert.NoError(t, err, "Failed to generate signed certificate")

	// get BLS public key
	blsPubKey, err := csp.GetBLSPublicKey(priv)
	assert.NoError(t, err, "Failed to generate signed certificate")
	assert.NotNil(t, blsPubKey, "Failed to generate signed certificate")

	// create our CA
	rootCA, err := ca.NewBLSCA(caDir, testCA3Name, testCA3Name, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")

	cert, err := rootCA.SignBLSCertificate(certDir, testName3, nil, nil, blsPubKey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.NoError(t, err, "Failed to generate signed certificate")
	// KeyUsage should be x509.KeyUsageDigitalSignature | x509.KeyUsageKeyEncipherment
...
func TestNewBLSCA(t *testing.T) {

	caDir := filepath.Join(testDir, "ca")
	rootCA, err := ca.NewBLSCA(caDir, testCAName, testCAName, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")
	assert.NotNil(t, rootCA, "Failed to return CA")
	assert.NotNil(t, rootCA.Signer,
		"rootCA.Signer should not be empty")
...
func TestGenerateRSASignCertificate(t *testing.T) {
	...
	caDir := filepath.Join(testDir, "ca")
	certDir := filepath.Join(testDir, "certs")
	adminDir := filepath.Join(testDir, "adminCerts")
	// generate private key
	...
	assert.NoError(t, err, "Failed to generate signed certificate")
	assert.NotNil(t, rsaPubKey, "Failed to generate signed certificate")

	adminPriv,_,err :=csp.GenerateRSAPrivateKey(adminDir)
	adminRSAPubkey,err :=csp.GetRSAPublicKey(adminPriv)
	...
	cert, err := rootCA.SignRSACertificate(certDir, testName, nil, nil, rsaPubKey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.NoError(t, err, "Failed to generate signed certificate")
	// KeyUsage should be x509.KeyUsageDigitalSignature | x509.KeyUsageKeyEncipherment
	...
	// make sure ous are correctly set
	ous := []string{"SampleOrg", "COP","OU_peer"}
	//cert, err = rootCA.SignRSACertificate(certDir, testName, ous, nil, rsaPubKey,
	//	x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	//assert.Contains(t, cert.Subject.OrganizationalUnit, ous[0])
	//assert.Contains(t, cert.Subject.OrganizationalUnit, ous[1])
	//
	//// make sure sans are correctly set
	sans := []string{testName2, testIP}
	//cert, err = rootCA.SignRSACertificate(certDir, testName, nil, sans, rsaPubKey,
	//	x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	//assert.Contains(t, cert.DNSNames, testName2)
	//assert.Contains(t, cert.IPAddresses, net.ParseIP(testIP).To4())


	cert, err = rootCA.SignRSACertificate(certDir, testName, ous, sans, rsaPubKey,
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	assert.Contains(t, cert.DNSNames, testName2)
	assert.Contains(t, cert.IPAddresses, net.ParseIP(testIP).To4())
	//sign admin cert
	sans2 := []string{"admin.com", "192.168.1.1"}
	cert, err = rootCA.SignRSACertificate(adminDir, "admin", ous, sans2, adminRSAPubkey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.Contains(t, cert.DNSNames, "admin.com")
	assert.Contains(t, cert.IPAddresses, net.ParseIP( "192.168.1.1").To4())
	...
	_, err = rootCA.SignRSACertificate(certDir, "empty/CA", nil, nil, rsaPubKey,
		x509.KeyUsageKeyEncipherment, []x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.Error(t, err, "Bad name should fail")
...
func TestGenerateBLSSignCertificate(t *testing.T) {
	...
	adminPriv,_,err :=csp.GenerateBLSPrivateKey(adminDir)
	adminBLSPubkey,err :=csp.GetBLSPublicKey(adminPriv)

	// create our CA
	rootCA, err := ca.NewBLSCA(caDir, testCA2Name, testCA2Name, testCountry, testProvince, testLocality, testOrganizationalUnit, testStreetAddress, testPostalCode)
	assert.NoError(t, err, "Error generating CA")
	...
	cert, err = rootCA.SignBLSCertificate(certDir, testName, ous, sans, blsPubKey,
		x509.KeyUsageDigitalSignature, []x509.ExtKeyUsage{})
	assert.Contains(t, cert.DNSNames, testName2)
	assert.Contains(t, cert.IPAddresses, net.ParseIP(testIP).To4())
	//sign admin cert
	sans2 := []string{"admin.com", "192.168.1.1"}
	cert, err = rootCA.SignBLSCertificate(adminDir, "admin", ous, sans2, adminBLSPubkey,
		x509.KeyUsageDigitalSignature|x509.KeyUsageKeyEncipherment,
		[]x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.Contains(t, cert.DNSNames, "admin.com")
	assert.Contains(t, cert.IPAddresses, net.ParseIP( "192.168.1.1").To4())
	...
	_, err = rootCA.SignBLSCertificate(certDir, "empty/CA", nil, nil, blsPubKey,
		x509.KeyUsageKeyEncipherment, []x509.ExtKeyUsage{x509.ExtKeyUsageAny})
	assert.Error(t, err, "Bad name should fail")
